class City < ActiveRecord::Base
	...
	def current_cinemas(movie)
		movie.cinemas.includes(:showtimes).where('showtimes.showing_at >= ? AND cinemas.id in (?)', Time.now-30.minutes, default_city? ? Cinema.all : cinemas).order('cinemas.name ASC').references(:showtimes).uniq!
	end
